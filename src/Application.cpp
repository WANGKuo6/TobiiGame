...
core::Application::~Application() {
	...
	utils::safe_delete(inputManager);
...
void core::Application::run(int argc, char *argv[]) {
    ...
    GazeObject gaze_object_(window_);
    ...
    while (true) {
		...
		gaze_object_.update(gaze_position);
...
void core::Application::init(int argc, char *argv[]) {
	...
	games_.push_back(new game::rpg::RapideGame(window_, cursor_movement_manager_, "Rapide", config_, musicManager));
	games_.push_back(new game::lg::LabyrintheGame(window_, cursor_movement_manager_, "Labyrinthe", config_, musicManager));
	// init other view
