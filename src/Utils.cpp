...
bool core::utils::isIntersect(int x, int y, SDL_Rect rect)
{
	return x > rect.x  && x < rect.x +  rect.w && y > rect.y  && y < rect.y +  rect.h;
}

bool core::utils::isInside(int paramx, int positionY, int x1, int y1, int levelNB) {
	int  x = x1 - 200 * sin(3.14*paramx / 180 / 5) + levelNB * 20;
	int y = y1 - 200 * sin(3.14*paramx / 180 / 5);
	if (x <= positionY - 50 && positionY + 50 <= y) {
		return true;
	}
	return false;
}
